...
// Delete a fragment's data from S3. Returns a Promise
async function deleteFragment(ownerId, id) {
  // Deleting the meta data from DynamoDB
  const dynamoParams = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    Key: { ownerId, id },
  };

  const deleteMetadataCommand = new DeleteCommand(dynamoParams);

  try {
    await ddbDocClient.send(deleteMetadataCommand);
  } catch (err) {
  ...
  // Deleting the data from S3 bucket
  const params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    Key: `${ownerId}/${id}`,
  };
